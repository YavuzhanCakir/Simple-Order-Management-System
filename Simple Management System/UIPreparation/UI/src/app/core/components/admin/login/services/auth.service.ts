...
export class AuthService {
  ...
  private userGroupLabels: string[] = [];

  getUserGroupNamesByUserId(): void {
    const userId = this.getCurrentUserId();
    this.httpClient
      .get<UserGroup[]>(environment.getApiUrl + `/user-groups/users/${userId}/groups`)
      .subscribe((groups) => {
        this.userGroupLabels = groups.map((group) => group.label);
        console.log("User groups:", this.userGroupLabels);
      });
  }

  isUserInGroup(groupName: string): boolean {
    console.log("includes?", this.userGroupLabels.includes(groupName));
    return this.userGroupLabels.includes(groupName);
  }
}
